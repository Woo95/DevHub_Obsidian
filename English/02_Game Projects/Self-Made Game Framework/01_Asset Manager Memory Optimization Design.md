...
| (3) memory block allocated with `malloc`, `CAssetManager` and `all resource managers` **created with placement new** | low compile dependency, no memory fragmentation | more complex to implement |
